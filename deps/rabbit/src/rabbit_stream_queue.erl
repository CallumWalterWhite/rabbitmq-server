...
consume(Q, Spec, QState0) when ?amqqueue_is_stream(Q) ->
    ...
    case check_queue_exists_in_local_node(Q) of
        ok ->
            ...
            %% TODO parse true/false properly
...
handle_event({osiris_chunk, _From, Tag, Chunks}, State = #stream_client{leader = Leader,
                                                                        readers = Readers0,
                                                                        name = Name}) ->
    #stream{pending_chunks = Chunks0} = Str0 = maps:get(Tag, Readers0),
    {ok, CRC} = osiris_server_sup:get_crc(Name),
    {Str, Msgs} = stream_entries_by_chunks(CRC, Tag, Leader,
                                           Str0#stream{pending_chunks = Chunks0 ++ Chunks}),
  ...
  when Credit > 0 ->
    case osiris_log:read_chunk_parsed(Seg0) of
        ...
        {[{Off, _} | _] = Records, Seg} when Off == ChunkId ->
            ...
            NumMsgs = length(Msgs),
            Str = Str0#stream{credit = Credit - NumMsgs, log = Seg, pending_chunks = Chunks},
